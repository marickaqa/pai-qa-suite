...
test.describe('Subtitles Editor', () => {
  ...
  test('should show cue list with at least one cue', async ({ page }) => {
    await page.goto(EDITOR_URL)
    await expect(page.locator('#1').or(page.getByText('#1'))).toBeVisible()
  })

  test('should show cue with start and end time inputs', async ({ page }) => {
    await page.goto(EDITOR_URL)
    await expect(page.locator('input[value="0,000"]').or(page.locator('input').first())).toBeVisible()
  })
  ...
  test('should show timeline waveform', async ({ page }) => {
    await page.goto(EDITOR_URL)
    await expect(page.locator('canvas, [class*="waveform"], [class*="timeline"]').first()).toBeVisible()
  })
