...
/**
 * This page has shown load-contention flakiness on three separate tests
 * across today's CI runs — always the same symptom (heading or its content
 * not rendered within the timeout), never the same test twice. Patching each
 * test's inline timeout individually was whack-a-mole. A single shared
 * navigation helper with a generous wait fixes it once for the whole file.
 */
async function gotoModelConfig(page: Page) {
  await page.goto(MODEL_CONFIG_URL)
  await expect(
    page.getByRole('heading', { name: 'Model & Logic' }),
    'model config page did not render'
...
test.describe('Core — SaaS Model & Logic', () => {
  ...
  test('should show Reset to defaults link in Output section', async ({ page }) => {
    await gotoModelConfig(page)
    await expect(page.getByText('Reset to defaults')).toBeVisible({ timeout: 15000 })
  })
